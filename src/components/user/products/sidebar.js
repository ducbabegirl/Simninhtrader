        ...
        <aside class="hidden lg:block lg:col-span-3 pt-3">
            <div>
                ...
                <ul class="grid grid-cols-1 divide-y mt-3">
                    ${cateList.map((item) => `
                        <li>
                            <a href="/#/category/${item.id}" class="${item.id === cateId ? "text-black font-semibold" : "text-[#ffbf00]"} block uppercase py-2 transition duration-300 ease-linear hover:text-black">${item.name}</a>
                        </li>
                        `).join("")}
                </ul>
            </div>
            ...
            <div class="mt-5">
                ...
                <ul class="grid grid-cols-1 divide-y mt-2">
                    ${productList.map((item) => /* html */ `
                        <li class="py-3 flex">
                            <a href="/#/product/${item.id}" class="block bg-[#ffbf00]">
                                <img class="w-16 h-16 object-cover block" src="${item.image}" alt="">
                            </a>

                            <div class="ml-3 flex-1">
                                <a href="/#/product/${item.id}" class="uppercase transition duration-300 ease-linear hover:text-black block text-[#ffbf00] text-sm">${item.name}</a>
